...
concatenation
"abc"
longueur chaine de caract
convertir string en entier et inversement
s = "12"

typage Dynamic : toutes les var peuvent changer de type comme en python
"""
...
| "print" "(" exp ")"                -> print
| "class" IDENTIFIER "{" constructor "}"   -> class
| lhs "=" exp ";"                               ->lhs
...
def pp_com(c):
  ...
  if c.data == "class":
    return f"class {c.children[0].value} " + "{" + f"{pp_constructor(c.children[1])}" + "\n}"
...
def asm_prg(p):
  ...
  DC = "\n".join([f"{c} : dq 0" for c in class_prg(p)])
...
def class_prg(p):
  return class_bcom(p.children[1])
